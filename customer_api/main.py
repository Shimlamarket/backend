...
async def google_auth(auth_request: GoogleAuthRequest):
    ...
    try:
        logger.debug(f"Received access token: {auth_request.access_token}")
        ...
        if not user:
            logger.info(f"User not found in database, creating new user with ID: {auth_result['user']['user_id']}")
            # Create new user
            ...
            logger.info(f"User found in database with ID: {user['user_id']}")
        
        logger.info("Returning authentication response")
        return AuthResponse(
